...
export const DashboardPage = () => {
  ...
  const delayedCases = patients.filter(
    (patient) => patient.status === "delayed",
  ).length;
  const averageWait = Math.round(
    patients.reduce((sum, patient) => sum + patient.waitingTime, 0) /
      patients.length,
  );
    ...
    <section className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MiniDashboard
          ...
          value={formatWaitTime(averageWait)}
          tone="danger"
        />
      </div>
      <Card>
        ...
        <ul className="space-y-2">
          {recent.map((patient) => (
            <li
              key={patient.id}
              className="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 dark:bg-slate-700/40"
            >
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-300">
                  {patient.maskedId} • {patient.currentStage}
                </p>
              </div>
              <Link
